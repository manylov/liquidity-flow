...
})
{
  ...
  const getProjectRouteById = async (id) =>{
    try {
      if (id){
        const projectRoute = await projectRouteService.getSingleDocumentById(id);
        return message.successResponse(
          { 'Content-Type': 'application/json' },
          responseCode.success,
          projectRoute
  ...
  const softDeleteProjectRoute = async (id) => {
    try {
      ...
      await projectRouteService.softDeleteDocument(id);
      let result = await deleteDependentService.softDeleteProjectRoute({ _id: id });
      return message.successResponse(
        { 'Content-Type': 'application/json' },
        responseCode.success,
        result);
